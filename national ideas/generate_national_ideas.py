...
def is_idea_groups_file(text):
    ...
    mil = 'category=mil'
    dff = 'default_ideas={'
    com = 'compatibility_127={'
    return adm in text or dip in text or mil in text or dff in text or com in text
    ...
    for filename in files:
        ...
        for line in lines:
            ...
            if parentheses_counter == 0 and open_parentheses > 0:
                ...
                in_trigger = True
                trigger += f'\t{line}'
            elif parentheses_counter == 1 and open_parentheses > 0 and not line.startswith('start=') and not line.startswith('bonus='):
                ideas.append(f'\t{line}')
            ...
            if parentheses_counter > 0 and parentheses_counter + open_parentheses - close_parentheses == 0:
                ideas_info.append((name, trigger, ideas))
                name = ''
                trigger = ''
                ideas = []
...
def generate_new_ideas(ideas_info, modifiers, multiplier, add_bonus, additinional_bonus_chance, negative_chance, file_name):
    ...
    for ideas in ideas_info:
        ...
        new_ideas.write('\tfree = yes\n')
        bonus1 = generate_bonus(modifiers, multiplier, negative_chance)
        bonus2 = generate_bonus(modifiers, multiplier, negative_chance)
        bonus3 = generate_bonus(modifiers, multiplier, negative_chance)
        new_ideas.write(f'\tstart = {{\n\t\t{bonus1[0]} = {bonus1[1]}\n\t\t{bonus2[0]} = {bonus2[1]}\n\t}}\n')
